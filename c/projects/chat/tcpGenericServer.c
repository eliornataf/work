...
static Status_t SetServerSocketOptval(int serverSocket);
static Status_t BindAndListenServer(struct sockaddr_in *serverAddress, int serverSocket, char *serverIPAddress, size_t serverPort, size_t queueLength);
static void SetMasterMonitorDescriptor(fd_set *masterMonitorDescriptor, int serverSocket);
static void ClientDestroy(void *clientSocket);
static Status_t AddNewClient(TCPServer_t *tcpServer, int newClientSocket);
static TCPServerError_t CheckNewClients(TCPServer_t *tcpServer);
static int ClientReceiveMessage(TCPServer_t *tcpServer, int clientSocket, void *buffer, size_t *numOfReadBytes);
static void CheckCurrClients(TCPServer_t *tcpServer, fd_set tempMonitorDescriptor, int numOfSignalSockets);

/*******************************************************
****************** Primary Functions *******************
*******************************************************/

TCPServer_t *CreateServer(char *_serverIPAddress, size_t _serverPort, size_t _queueLength, size_t _maxConnectClients, NewClientFunc_t _newClientFunc, CloseClientFunc_t _closeClientFunc, ClientMessageFunc_t _clientMessageFunc, ErrorFunc_t _errorFunc, void *appContext)
{
    int serverSocket;
    ...
    if (!_serverIPAddress || (_serverPort < MIN_PORT_NUM || MAX_PORT_NUM < _serverPort) || !_queueLength ||  DESCRIPTORS_LIMIT < _maxConnectClients || !_clientMessageFunc)
    {
        return NULL;
    }
    ...
    tcpServer->m_appContext = appContext;
...
void RunServer(TCPServer_t *_tcpServer)
{
    ...
    while (1)
    {
        ...
        {
            if (!_tcpServer->m_errorFunc(_tcpServer, TCP_SERVER_CRITICAL_ERROR, _tcpServer->m_appContext))
            {
                break;
            }
        }
        ...
        if (FD_ISSET(_tcpServer->m_serverSocket, &tempMonitorDescriptor))
        {
            ...
            {
                if (!_tcpServer->m_errorFunc(_tcpServer, statusCode, _tcpServer->m_appContext))
                {
                    break;
                }
            }
...
int ServerSendMessage(TCPServer_t *_tcpServer, int _clientID, void *_message, size_t _messageSize)
{
    ...
    {
        _tcpServer->m_errorFunc(_tcpServer, TCP_SERVER_SEND_MESSAGE_FAILED, _tcpServer->m_appContext);
...
static TCPServerError_t CheckNewClients(TCPServer_t *_tcpServer)
{
    ...
    if (_tcpServer->m_newClientFunc)
    {
        ...
        _tcpServer->m_newClientFunc(_tcpServer, &tcpClient, _tcpServer->m_appContext);
    }
...
{
    *_numOfReadBytes = recv(_clientSocket, _buffer, BUFFER_LENGTH, 0);

    if (RECV_ERROR == *_numOfReadBytes)
    {
        return CLIENT_ERROR;
    }
...
static void CheckCurrClients(TCPServer_t *_tcpServer, fd_set _tempMonitorDescriptor, int _numOfSignalSockets)
{
    ...
    while (!ListIterEqual(iterRunner, iterEnd))
    {
        ...
        if (FD_ISSET(*(int*)currClient, &_tempMonitorDescriptor))
        {
            ...
            if ( (CLIENT_ERROR == recvStatus) || (CLIENT_FINISH == recvStatus) )
            {
                ...
                {
                    _tcpServer->m_closeClientFunc(_tcpServer, *(int*)currClient, _tcpServer->m_appContext);
                }
            ...
            {
                _tcpServer->m_clientMessageFunc(_tcpServer, *(int*)currClient, buffer, _tcpServer->m_appContext);
